...
In the _Optiflow–CloudSync CRM_ integration, CloudSync supplies customer engagement, ticket data, and real-time visibility updates from the CloudSync CRM system to the Optiflow system. The Optiflow–CloudSync CRM integration uses a _push_ method, where data is pushed to Optiflow by CloudSync. _No data is sent from Optiflow back to CloudSync_.
...
|:-----|:-----------------------------|:---------------------------------------------------------------------------|
|1     |[Enable change data capture](#enable-change-data-capture)| If you are processing the STT and CIM APIs, select the relevant CDC (Change Data Capture) entities for the integration, which can include: <br> - **ChatSession** <br> - **SupportTicket** <br> - **VideoSession** <br> - **UserPresence** |
...
4. From **Available Entities**, move the relevant CDC entities that are in scope for the integration to **Selected Entities**, which can include: <br> - **SupportTicket** <br> - **ChatSession** <br> - **VideoSession** <br> - **UserPresence** 
